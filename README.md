...
Five views, tied together by a shared audio pipeline:
...
**Chromatic as a key, not a mode** — a chromatic scale has no tonic and no key signature, so it's offered as a thirteenth entry in the *key* selector rather than alongside major and minor; picking it disables the mode selector. It's anchored on C, which spells cleanly ascending, over one or two octaves — or **Full range**, which runs every charted note from B♭3 to A6 and back (71 notes), with the staff scrolling to keep the current note on screen.
